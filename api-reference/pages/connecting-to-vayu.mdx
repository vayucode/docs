...
ACCESS_TOKEN=$(curl -s -X POST "https://api.withvayu.com/v1/auth/login" \
  -H "Content-Type: application/json" \
  -H "x-api-key: $VAYU_CLIENT_ID" \
  -d "{\"token\": \"$VAYU_API_TOKEN\"}" | jq -r '.accessToken')
...
<Note>
Access tokens expire after **1 hour**. The Vayu SDKs handle token refresh automatically. If you're using a raw HTTP client, re-authenticate when you receive a `401` response.
</Note>