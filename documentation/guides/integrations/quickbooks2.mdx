...
- **Invoice Status Updates**: Retrieves updated status (Paid, Overdue, Voided) from QuickBooks to sync back into Vayu.
...
The Vayu QuickBooks integration can handle up to 500 requests/minute (including retries). It also comes replete with rich support for Chart of Accounts (COA, mapping Vayu products to QuickBooks accounts) and segmentation by classes and location. 
