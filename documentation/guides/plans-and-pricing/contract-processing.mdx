...
- **Expired** A contract automatically moves to **Expired** once its end date has passed. No new invoices are generated for an expired contract. You can bring an expired contract back into service by extending its end date — see [Extend Contract End Date](#extend-contract-end-date) below.
...
Extend the end date of an existing contract to keep billing running for an additional period.

**When to use**

- The customer renews or continues service past the original end date.
- A contract has already moved to **Expired** and you want to resume billing without recreating it.

**What to expect**

1. The new end date is applied at the end of the selected day (UTC), so the contract remains active for the full final day.
2. If the contract was **Expired**, it automatically transitions back to **Active** once the new end date is in the future.
3. Any invoices that were affected by the previous end date and are no longer valid for the extended period are removed and regenerated based on the new terms.
